    ...
    execute: function () {
      ...
      _export("MockApi", MockApi = class MockApi {
        ...
        static play() {
          ...
          var reels = (_this$forcedNext = this.forcedNext) != null ? _this$forcedNext : this.buildRandomReels();
          this.forcedNext = null;
          var winLines = this.evaluateWins(reels);
          var totalWin = winLines.reduce((sum, wl) => sum + wl.winAmount, 0);
          var payload = {
        ...
        static evaluateWins(reels) {
          ...
          for (var line of PAYLINES) {
            var _PAYTABLE$symbolId;

            var symbols = [];
            var indexes = []; // Collect symbols for this line. line.rows are payline-space (0..2);
            // physical reel rows are offset by VISIBLE_ROW_OFFSET to skip the hidden upper row.

            for (var reel = 0; reel < REELS; reel++) {
              var paylineRow = line.rows[reel];
              var physicalRow = paylineRow + VISIBLE_ROW_OFFSET;
              symbols.push(reels[reel][physicalRow]);
              indexes.push(this.toIndex(reel, paylineRow));
            }

            var evaluation = this.evaluateLine(symbols);
            ...
            var payout = (_PAYTABLE$symbolId = PAYTABLE[symbolId]) == null ? void 0 : _PAYTABLE$symbolId[count];
