    ...
    execute: function () {
      ...
      _export("WinLines", WinLines = (_dec = ccclass('WinLines'), _dec2 = property({
        ...
        onLoad() {
          ...
          for (var label of this.allPerLineLabels()) {
            this.restScales.set(label, label.node.scale.clone());
            label.node.active = false;
          }
        }

        playWinLines(winLines) {
          var _this = this;
          ...
          return _asyncToGenerator(function* () {
            ...
            for (var i = 0; i < winLines.length; i++) {
              ...
              _this.hideAllPerLineLabels();

              if (i > 0) {
                yield _this.delay(_this.delayBetweenLines);
              }

              var lineNode = _this.winLinesRoot.getChildByName(line.lineNumber.toString());

              if (lineNode) {
                lineNode.active = true;
              }

              _this.showPerLineLabel(_this.getLabelForLine(line.lineNumber), line.winAmount);

              _this.playSymbolBounce(line.nodes);

              runningTotal += line.winAmount;

              _this.tweenTotalTo(runningTotal, _this.lineDisplayTime / 1000);

              yield _this.delay(_this.lineDisplayTime);
        ...
        update() {
          ...
          var shown = Math.floor(this.displayedTotal);
        ...
        playSymbolBounce(nodes) {
          for (var node of nodes) {
            ...
            var enlarged = new Vec3(originalScale.x * this.symbolScale, originalScale.y * this.symbolScale, originalScale.z);
        ...
        hideAllLines() {
          for (var child of this.winLinesRoot.children) {
            child.active = false;
          }
        }
        ...
        getLabelForLine(lineNumber) {
          var pos = WinLines.LINE_TO_POSITION[lineNumber];
        ...
        showPerLineLabel(label, amount) {
          if (!label) return;
          var restScale = this.restScales.get(label);
          if (!restScale) return;
          label.string = "" + amount;
