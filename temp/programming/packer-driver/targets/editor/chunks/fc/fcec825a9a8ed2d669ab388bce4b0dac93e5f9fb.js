    ...
    execute: function () {
      ...
      _export("Main", Main = (_dec = ccclass('Main'), _dec2 = property({
        ...
        async onPlayPressed() {
          ...
          this.winLines.resetTotal(); // Fire API in parallel with the spin animation.
        ...
        async handlePlayResponse(data) {
          ...
          const linesWithNodes = data.winLines.map(line => ({
            ...
            nodes: line.winIndexes.map(idx => this.reelController.getCellByIndex(idx)).filter(n => n !== null)
          }));
        ...
        applyResponsiveResolution() {
          ...
          const policy = w / h >= Main.DESIGN_W / Main.DESIGN_H ? ResolutionPolicy.FIXED_HEIGHT : ResolutionPolicy.FIXED_WIDTH;
